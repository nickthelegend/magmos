...
await step('Contracts compile and every test passes', async () => {
  ...
  if (Number(m[2]) === 0) ok(`${m[1]} Foundry tests pass`)
  else bad(`${m[2]} Foundry tests failing`)
})
...
await step('Unit tests', async () => {
  ...
  if (Number(m[2]) === 0) ok(`${m[1]} TypeScript tests pass`)
  else bad(`${m[2]} failing`)
})
...
await step('Deployed contracts are live on Arc', async () => {
  for (const [name, addr] of Object.entries(dep)) {
    ...
    if (code && code !== '0x') ok(`${name} has bytecode at ${addr}`)
...
await step('Addresses and ABIs have not drifted', async () => {
  ...
  if (out.includes('✓')) ok('deployment record matches app + docs')
  else bad(out.trim().slice(0, 200))
})

await step('Privacy holds, from public chain data only', async () => {
  try {
    const out = run('node scripts/verify-privacy.mjs', APP)
    if (out.includes('employee identities recoverable      : NO')) {
      ok('no employee identity recoverable from any settlement or delivery')
    } else {
      bad('privacy verification did not report NO')
    }
    if (out.includes('0xba1f74a9')) ok('known-leaky control still detected as leaking')
    else bad('control no longer flags — the check may be inert')
  } catch (e) {
...
await step('No mocks left in shipped code', async () => {
  ...
  if (out.trim()) bad(`found: ${out.trim().split('\n')[0].slice(0, 140)}`)
  else ok('no mock/fake/dummy/fixme in shipped code')
})
